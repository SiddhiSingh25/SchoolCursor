...
import { deleteEvent, listEvents } from "./events-service";
...
export function EventsTable() {
  ...
  const query = useQuery({
    queryKey: ["admin", "events", { q, page }],
    queryFn: () => listEvents({ q, page, pageSize: 10 }),
  });
  ...
  return (
    <Card>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Manage events shown on the public website.
            </p>
          </div>
            ...
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="border-b text-xs text-muted-foreground">
                <tr>
                  ...
                  <th className="py-3 pr-4">Event date</th>
                ...
                {query?.data?.rows?.map((row) => (
                  <tr key={row.id} className="border-b last:border-b-0">
                    ...
                    <td className="py-3 pr-4 text-muted-foreground">
                      {row.event_date ? row.event_date.slice(0, 10) : "—"}
                    </td>
                    <td className="py-3 pr-4 text-muted-foreground">
                      {row.location ?? "—"}
                    </td>
      ...
      <ConfirmDialog
        ...
        title="Delete event?"
        description="This will permanently remove the event from the website."
        confirmText="Delete"
        destructive
        busy={del.isPending}
        onConfirm={() => {
